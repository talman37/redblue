...
class DmContextGenerator {
    fun defaultExecutiveExpireDmGenerate(
        company: Company,
        executives: List<Executive>?,
        user: LawFirmUser,
        master: List<Executive>?
    ): Context {
        ...
        return Context().apply {
            ...
            company.companyName?.forEach {
                if (it.toInt() in 0x4E00..0x9FFF) {
                    isChineseCharacter = true
                }
            }
            this.setVariable("isChineseCharacter", isChineseCharacter)

            if (company.companyName!!.contains("株式會社")) {
            ...
            this.setVariable(
                "etcContent1",
                "임기만료일로부터 2주 이내에 <b style=\"text-decoration: underline;\">임원변경등기를 하지 않으면 상법 제635조 제1항에 따라 500만원 이하의 과태료가 부과</b>되오니 각별히 유의하시기 바랍니다."
            )
            this.setVariable(
                "etcContent2",
                "대표자(대표이사 또는 1인 사내이사)의 개인주소가 변경된 경우도 2주 이내에 변경등기를 하여야 합니다. 이를 위반하여 과태료를 부과 받는 회사들이 많으니, 특히 유의하시기 바랍니다."
            )
            this.setVariable("etcContent3", "위 정보는 공개된 법인등기부등본을 기초로 작성된 것이므로, 개인정보를 침해한 사실이 전혀 없음을 밝혀 드립니다.")
            this.setVariable(
                "content3",
                "<span class=\"font-other\">담당자 이메일(lawn6616@naver.com) 또는 팩스로 법인등기부등본, 주주명부, 정관 사본을 미리 보내주시면, 보다 신속하고 정확한 상담 및 서류 안내가 가능합니다.</span>"
            )
            this.setVariable(
                "desc",
                "\"당사무소는 법인등기 외 가압류, 가처분, 소송, 부동산등기 업무 등 각종 법률사무를 취급하오니, 이에 대하여 귀사의 문의가 있을 경우 정성과 책임을 다하여 상담하여 드리겠습니다.\""
            )
        }
    }


    fun lawAndOfficeExecutiveExpireDmGenerate(
        company: Company,
        executives: List<Executive>?,
        user: LawFirmUser,
        master: List<Executive>?
        ...
        return Context().apply {
            ...
            this.setVariable(
                "conTitleName",
                "<strong>" + company.companyName + " " + masterInfo?.get(0)?.name + " 대표님</strong><span> 귀하</span>"
            )
            var box =
                "<strong>○<b>인감증명서 창구 발급용</b>만 사용가능</strong><br/><span>(전자 발급용 사용불가)</span><br/><strong>○<b>온라인 전자신청</b> 처리 가능</strong><br/><span>(당사자 인증 필요)</span>"
            var warning =
                "<strong>임기만료, 대표님의 주소변경</strong>으로 인한 변경등기는 <strong>2주</strong> 이내에 하셔야 하며, 등기를 늦게 할 경우 상법상 과태료(<em>최고 500만원까지</em>)가 <em>대표님 개인에게 부과</em>될 수 있습니다."
            var content0 = "귀사의 무궁한 발전을 기원합니다."
            ...
            this.setVariable(
                "bottom3",
                "기업법무 / 출자전환 / 현물출자 / 가수금증자 / 자기주식취득 / 부동산등기(취득세) <br/> 개인회생&middot;파산 / 채권관리&middot;추심 / 압류&middot;추심 / 성년후견 / 유언(상속)대용신탁"
            )
        }
    }
